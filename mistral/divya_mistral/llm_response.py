...
def extract_label(output):
    ...
    for a in allowed:
        if re.match(rf"^{re.escape(a)}\b", first_line, re.IGNORECASE):
            return a
    return "Incorrect"
df = pd.read_excel("output_without_prompt_mndi_mist.xlsx")  # or pd.read_json("your_output.jsonl", lines=True)
df["extracted_label"] = df["llm_response"].apply(extract_label)
df.to_excel("output_with_labels_mndi.xlsx", index=False)
